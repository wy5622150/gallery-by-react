...
class AppComponent extends React.Component {
	constructor(props) {
		super(props);
		// 这里的constant规定了图片在stage中的position横向纵向坐标取值范围，下面将其初始化
		this.Constant = {
			centerPos: {
				left: 0,
				right: 0
	 ...
	 */
	// 整体过程是随机抽出一个占中间位，然后抽0-1个在上部分，剩下的左右均摊
	// 该方法中有setState
	rearrange(centerIndex) {
		...
		//取出要布局上侧的图片的状态信息
		topImgSpliceIndex = Math.ceil(Math.random() * (imgsArrangeArr.length - topImgNum)); //避免index取值溢出数组，index是起始位置
	...
	componentDidMount() {
		...
		// 左右侧图片区域排布取值
		// 从这里开始为Constructor区里面的this.Constant的各属性“赋值”，利用前面“留用”的值
		...
		// 三个区域的图片放置范围至此确定
		var num = Math.floor(Math.random() * 10)  //取一个0-10的随机数
	...
	render() {
		...
		imageDatas.forEach(function (value, index) {
			...
			// imgFigures是一个ImgFigure函数返回值型的数组，ImgFigure返回值是包含图片以及说明文字组成的jsx
			// imgFigures实质上是给ImgFigure做了一个数组式的打包
